...
def analytical_eigenvalues(mode,mode2):
    ...
    mu_b=m_atm/(rho*S*b)
    C_L = 2 * W / (rho * V ** 2 * S)
    C_X_0    = W * np.sin(th0) / (0.5 * rho * V ** 2 * S)
    C_Z_0    = -W * np.cos(th0) / (0.5 * rho * V ** 2 * S)
    if mode == 'phugoid':
        ...
        return root_solver(A,B,C) *V_calib/c
        ...
        return C_l_p / (4*mu_b * K_XX**2) *V_calib/b
    elif mode == 'dutch_roll':
        A= 8 * mu_b**2 *K_ZZ**2
        B= -2 * mu_b * (C_n_r + 2*K_ZZ**2*C_Y_beta)
        C= 4*mu_b*C_n_beta + C_Y_beta * C_n_r
        return root_solver(A,B,C)*V_calib/b
    elif mode == 'spiral':
        return 2*C_L * (C_l_beta*C_n_r - C_n_beta* C_l_r) / (C_l_p* (C_Y_beta * C_n_r + 4*mu_b*C_n_beta) - C_n_p * (C_Y_beta*C_l_r+4 * mu_b *C_l_beta)) * V_calib/b
    else:
        return print('You have not entered a valid mode')
    
def state_system(hp0,V,alpha0,th0,m):
    V_calib=V
    W=m * 9.81
    rho=rho0 * np.power( ((1+(lambda_grad * hp0 / Temp0))), (-((g / (lambda_grad*R)) + 1)))
    mu_c=m/(rho*S*c)
    ...
    S1=np.array([[-2*mu_c*c/V_calib,0,0,0],
                 [0,(C_Z_alpha_dot-2*mu_c)*c/V_calib,0,0],
                 [0,0,-c/V_calib,0],
                 [0,C_m_alpha_dot*c/V_calib,0,-2*mu_c*K_YY**2*c/V_calib]])
    ...
    A1=np.array([[(C_Y_beta_dot-2*mu_b)*b/V_calib,0,0,0],
                 [0,-1/2*b/V_calib,0,0],
                 [0,0,-4*mu_b*K_XX**2*b/V_calib,4*mu_b*K_XZ*b/V_calib],
                 [C_n_beta_dot*b/V_calib,0,4*mu_b*K_XZ*b/V_calib,-4*mu_b*K_ZZ**2*b/V_calib]])
...
def data_calls(mode):
    if mode == 'generic':
        source= loadmat('matlabR.mat')
    elif mode == 'our_data':
        source= loadmat('FTISxprt-20200309_flight1.mat')
    else:
        return print('You have not entered a valid mode')
    flight_data=source['flightdata']
    calls = flight_data.dtype.calls
    return calls    
    
def data(call):
    data=loadmat('FTISxprt-20200309_flight1.mat')
    flight_data=data['flightdata']
    values= flight_data[0][0][call]['data'].flat[0]
    units = flight_data[0][0][call]['units'].flat[0].flat[0].flat[0]
    description = flight_data[0][0][call]['description'].flat[0].flat[0].flat[0]
...
def data_processing(mode1,mode2):
    ...
    vector=data(data_calls(mode2)[48])[0]
    if mode2 == 'generic':
