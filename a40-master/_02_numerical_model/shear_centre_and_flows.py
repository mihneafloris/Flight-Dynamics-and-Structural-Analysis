...
class Shear_flow_and_centre:
    ...
    def shear_centre_and_flow_y_dir(self,data_dict,S_y,I_zz):
        ...
        for i in range(100):
            qb1_persection.append(- S_y * self.t_sk/I_zz* self.h**2 * integrate.quadrature(np.sin, r1_array[i], r1_array[i+1])[0])
            qb1_uptopoint.append(-S_y*self.t_sk/I_zz* self.h**2 * integrate.quadrature(np.sin, r1_array[0], r1_array[i+1])[0])
        #Region 2
        ...
        for i in range(100):
            qb3_persection.append(-S_y*self.t_sk/I_zz*integrate.quadrature(f,r3_array[i],r3_array[i+1])[0]+qb1_uptopoint[99]+qb2_uptopoint[99])
            qb3_uptopoint.append(-S_y*self.t_sk/I_zz*integrate.quadrature(f,r3_array[0],r3_array[i+1])[0]+qb1_uptopoint[99]+qb2_uptopoint[99])
        #Region 4
        qb4_persection=[]
        qb4_uptopoint=[]
        f=lambda s4: -self.h/self.l_sk * s4
        for i in range(100):
            qb4_persection.append(-S_y*self.t_sk/I_zz*integrate.quadrature(f,r4_array[i],r4_array[i+1])[0]+qb3_uptopoint[99])
            qb4_uptopoint.append(-S_y*self.t_sk/I_zz*integrate.quadrature(f,r4_array[0],r4_array[i+1])[0]+qb3_uptopoint[99])
        #Region 5
        qb5_persection=[]
        qb5_uptopoint=[]
        f=lambda y: y
        ...
        qb6_uptopoint=[]
        for i in range(100):
            qb6_persection.append(-S_y*self.t_sk/I_zz* self.h**2 * integrate.quadrature(np.sin, r6_array[i], r6_array[i+1])[0]+qb4_uptopoint[99]-qb5_uptopoint[99])
            qb6_uptopoint.append(-S_y*self.t_sk/I_zz* self.h**2 * integrate.quadrature(np.sin, r6_array[0], r6_array[i+1])[0]+qb4_uptopoint[99]-qb5_uptopoint[99])
    ...
    def shear_flow_z_dir(self,data_dict,S_z,I_yy,S_y,I_zz,z_c):
        ...
        qb1_persection=[] #this is the base shear flow for region 1 for each discrete length
        qb1_uptopoint=[]  #this is the base shear flow from the beginning of the region up until a discrete length
        f= lambda theta: -(1-np.cos(theta))*self.h-z_c 
        for i in range(100):
        ...
        qb3_persection=[]
        qb3_uptopoint=[]
        f=lambda s3: (-self.h-z_c)-(self.C_a-self.h)/self.l_sk*s3
        for i in range(100):
            qb3_persection.append(-S_z*self.t_sk/I_yy*integrate.quadrature(f,r3_array[i],r3_array[i+1])[0]+qb1_uptopoint[99]+qb2_uptopoint[99])
            qb3_uptopoint.append(-S_z*self.t_sk/I_yy*integrate.quadrature(f,r3_array[0],r3_array[i+1])[0]+qb1_uptopoint[99]+qb2_uptopoint[99])
        #Region 4
        qb4_persection=[]
        qb4_uptopoint=[]
        f=lambda s4: (-self.C_a-z_c)+(self.C_a-self.h)/self.l_sk*s4
        for i in range(100):
            qb4_persection.append(-S_z*self.t_sk/I_yy*integrate.quadrature(f,r4_array[i],r4_array[i+1])[0]+qb3_uptopoint[99])
            qb4_uptopoint.append(-S_z*self.t_sk/I_yy*integrate.quadrature(f,r4_array[0],r4_array[i+1])[0]+qb3_uptopoint[99])
        #Region 5
        qb5_persection=[]
        qb5_uptopoint=[]
        f=lambda y: -self.h-z_c
        ...
        f= lambda theta: -(1-np.cos(theta))*self.h-z_c 
        for i in range(100):
            qb6_persection.append(-S_z*self.t_sk/I_yy* self.h**2 * integrate.quadrature(np.sin, r6_array[i], r6_array[i+1])[0]+qb4_uptopoint[99]-qb5_uptopoint[99])
            qb6_uptopoint.append(-S_z*self.t_sk/I_yy* self.h**2 * integrate.quadrature(np.sin, r6_array[0], r6_array[i+1])[0]+qb4_uptopoint[99]-qb5_uptopoint[99])
    ...
    def plot_shearflowdistributions(self,data_dict,M_x,S_z,S_y,I_zz,I_yy,z_c):
        #Coordinate and s arrays
        r1_array,r1_coord_z,r1_coord_y,r2_array,r2_coord_z,r2_coord_y,r3_array,r3_coord_z,r3_coord_y,r4_array,r4_coord_z,r4_coord_y,r5_array,r5_coord_z,r5_coord_y,r6_array,r6_coord_z,r6_coord_y=Shear_flow_and_centre.region_geometry(self)
        q1_tot,q2_tot,q3_tot,q4_tot,q5_tot,q6_tot=Shear_flow_and_centre.sum_shearflowdistributions(self,data_dict,M_x,S_z,S_y,I_zz,I_yy,z_c)

        ### Plot region 1
        x1 = r1_coord_z
        y1 = r1_coord_y
        z1 = [x*self.t_sk for x in q1_tot]
        ...
        x2 = r2_coord_z
        y2 = r2_coord_y
        z2 = [x*self.t_sp for x in q2_tot]
        ...
        x3 = r3_coord_z
        y3 = r3_coord_y
        z3 = [x*self.t_sk for x in q3_tot]
        ...
        x4 = r4_coord_z
        y4 = r4_coord_y
        z4 = [x*self.t_sk for x in q4_tot]
        ...
        x5 = r5_coord_z
        y5 = r5_coord_y
        z5 = [x*self.t_sp for x in q5_tot]
        ...
        x6 = r6_coord_z
        y6 = r6_coord_y
        z6 =[x*self.t_sk for x in q5_tot]
