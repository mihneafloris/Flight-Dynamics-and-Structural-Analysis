...
class Moments_of_inertia:
    ...
    def calculate_centroid(self):
        ...
        self.totalarea_dis = 2*self.ha/m.pi/2 * self.arc + 2*-1*(self.ca-self.ha/2)/2*self.sheet + 0*self.spar#Area distance from skin and spar
        ...
        for i in range((self.nst-self.ncirc)//2): #Stringers on the sheet
            z = self.ca - self.ha/2 - i*self.stringerspacing*m.cos(self.sheetangle) - self.stringerspacing/2*m.cos(self.sheetangle)
    ...
    def calculate_moments_of_inertia_izz(self, data_dict):
        ...
        r = self.ha/2
        l = self.ca-r
        self.i_zz = m.pi*r**3*self.tsk/2 + 2*r**3*self.tsp/3 + self.tsk*l**3/12*(2*r/m.sqrt(l**2 + r**2))**2
    ...
    def calculate_moments_of_inertia_iyy(self, data_dict):
        ...
        self.i_yy = m.pi*self.tsk*r**3/2 + m.pi*r*self.tsk*(2*r/m.pi - self.centroid_z)**2 + \
                    self.tsk*(r**2+l**2)**(3/2)*l/(6*r) + 2*self.tsk*m.sqrt(r**2+l**2)*(-l/2-self.centroid_z)**2
        for i in range(len(self.stringerlocations_z)): #append per stringer
    ...
    def plot(self):
        ...
        plt.plot([-self.ca+self.ha/2, 0],[0,self.ha/2],[-self.ca+self.ha/2, 0],[0,-self.ha/2],[0,0],[self.ha/2,-self.ha/2]) #Sheets
        #Creation of the arc
        z = []
        y = []
        for i in range(31):
